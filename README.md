...
```mermaid
flowchart TD
    User[("Homeowner enters<br/>US address")] --> SSE
    ...
    Advisor[Advisor Agent<br/>verified product catalog] --> Dossier[Personalized Dossier]
    Dossier --> User
```

Seven specialist agents run on **Gemma 4** via OpenRouter:

- **5 data agents** fan out concurrently against free public APIs.
  Each is an LLM agent that fetches raw data, then asks Gemma 4 to
  interpret it into a structured finding with a 1-sentence summary.
...
Sections, ordered for the mission (action first, math last):
...
| LLM gateway | OpenRouter free tier + BYOK | $0, OpenAI-compatible API, easy fallback |
| Backend | Python 3.13 + FastAPI + uvicorn + httpx | Async-native, SSE-friendly |
| Frontend | React via Babel-standalone (single `index.html`) | No build step, deploys anywhere |
...
| Hosting (planned) | HF Spaces (backend, Docker SDK) + Cloudflare Pages (frontend) | Both free |
...
│   │   └── advisor_agent.py   catalog-driven, no inventing
│   ├── tools/
...
- **Native function calling** — verified end-to-end on the `:free`
  tier (see `backend/scripts/smoke_test.py`)
...
- **Agentic workflows** — 7 agents, parallel-then-sequential
